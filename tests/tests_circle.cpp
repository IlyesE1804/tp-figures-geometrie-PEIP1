...
// Commande pour créer un exécutable test.exe :
...
void test_cas_simples() {
    ...
    cout << "Tous les tests simples du cercle ont réussi avec succès. " << endl;
}
...
void test_cas_limites() {
    ...
    // Ces appels sont supposés être désactivés si resize(<0) est interdit
    // c5.resize(0);   // ne doit pas modifier le rayon
    // c5.resize(-10); // ne doit pas modifier le rayon
    assert(c5.equals(c6) == true);

    // Circle crayon_negatif(-5, Point(1,1)); // probablement interdit

    cout << "Tous les tests limites du cercle ont réussi avec succès. " << endl;
}
